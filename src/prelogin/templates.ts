...
interface BasePreloginTemplateOptions {
    contents: string
    language: string
...
export const basePreloginTemplate = ({
    ...
    footerLinks = [],
    settings = {}
}: BasePreloginTemplateOptions): string => `
<div id="fw-wait">
...
${
    settings?.BRANDING_LOGO
        ? `<div class="fw-login-branding-logo"><img src="${staticUrl(settings.BRANDING_LOGO as string)}" alt="Brand logo" /></div>`
        : ""
}
   </div>
   ${
