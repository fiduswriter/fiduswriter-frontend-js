...
export const confirmDeleteAvatarTemplate = (): string =>
    `<p>${gettext("Delete the avatar?")}</p>`

/** A template to change the user avatar. */
export const changeAvatarDialogTemplate = (): string =>
...
export const deleteUserDialogTemplate = (): string =>
    `<h3>
...
export const changeEmailDialogTemplate = (): string =>
...
export const deleteEmailDialogTemplate = ({text}: {text: string}): string =>
    `<p>${escapeText(text)}</p>`

/** A template for the change primary email dialog of the user account. */
export const changePrimaryEmailDialogTemplate = ({text}: {text: string}): string =>
    `<p>${escapeText(text)}</p>`

/** A template for the change password dialog of the user account. */
export const changePwdDialogTemplate = ({username}: {username: string}): string =>
    `<table class="ui-dialog-content-table"><tbody>
        ...
        <div id="profile-data">
            ...
            <div class="profile-data-row">
                <table class="fw-data-table profile-email-table">
                    ...
                    <tbody>
                        ${(user.emails as Array<Record<string, unknown>>)
                            .map(
                                (email: any) => `<tr${email.primary ? ' class="primary-email-tr"' : ""}>
            ...
            <div class="profile-data-row">
                ...
                ${((settings.LANGUAGES as Array<[string, string]>) || [])
                    .map(
                        ([code, _name]) =>
